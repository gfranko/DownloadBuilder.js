...
(function (w, d, undefined) {
    ...
    downloadBuilder.prototype = {
        ...
        _isUsingGithub: function() {
            ...
            var githubSelector = d.querySelectorAll('[data-location="github"]');
        ...
        JSONP: function(url,method,callback) {
            ...
            d.getElementsByTagName("head")[0].appendChild(jsonpScript);
        },
        ...
        buildURL: function(checkboxes, fileName, lang, callback) {
            ...
            if(checkboxes.length) {
                ...
                location = currentElem.getAttribute("data-location") || "local";
                ...
                if(window.sessionStorage) {
                    ...
                    if(window.sessionStorage.getItem(currentElem.value) !== undefined && window.sessionStorage.getItem(currentElem.value + ":time") !== undefined) {

                         // If the caching time has expired
                        if((time - window.sessionStorage.getItem(currentElem.value + ":time")) >= self.options.cache) {

                            // Removes the session data
                            window.sessionStorage.removeItem(currentElem.value);

                            // Removes the session data
                            window.sessionStorage.removeItem(currentElem.value + ":time");
        ...
        _localRequest: function(obj) {
            ...
            // LOCAL VARIABLES
            var self = this,
                sessionItem = window.sessionStorage.getItem(obj.elem.value),
                xhr,
                text;
            ...
            try {
                ...
                xhr.onreadystatechange = function(e) {
                    ...
                    if (this.readyState === 4 && this.status === 200) {
                        ...
                        self._setSessionData({ "elemValue": obj.elem.value, "data": text, "time": obj.time });
        ...
        _thirdPartyRequest: function(obj) {
            ...
            var self = this,
                // Holds the value of whatever is currently stored in the session (Defaults to an empty string)
                sessionItem = window.sessionStorage.getItem(obj.elem.value) || "";
            ...
            this.JSONP(obj.url, function(data) {
                ...
                if(obj.type === "github") {
                    ...
                    self.file += self._parseGithubResponse({ "elem": obj.elem, "data": data }) || "";

                }

                // If there is not any session data currently stored
                if(!sessionItem) {

                    // Stores all the necessary data inside of the session
                    self._setSessionData({ "elemValue": obj.elem.value, "data": obj.text, "time": obj.time });
        ...
        _setSessionData: function(obj) {

            window.sessionStorage.setItem(obj.elemValue, obj.data || "");

            window.sessionStorage.setItem(obj.elemValue + ":time", obj.time);

        },

        // createURL
        // ---------
        //      Constrcuts a file url using the HTML5 Filesystem API (Only currently available in Chrome 12+)
